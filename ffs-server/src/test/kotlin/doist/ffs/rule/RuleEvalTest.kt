...
class RuleEvalTest {
    ...
    @Test
    fun testMissingEquals() {
        assertThrows<ParseException> { eval("""abc""") }
        assertThrows<ParseException> { eval(""""abc"""") }
        assertThrows<ParseException> { eval("""now()""") }
        assertThrows<ParseException> { eval("""contains([1, 2], 1)""") }
        assertThrows<ParseException> { eval("""1/2""") }
    }
